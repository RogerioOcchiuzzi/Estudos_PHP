...
class Usuarios extends Model
{
    ...
    public function cadastro($nome, $apelido, $senha, $email) 
    {
        ...
        if ($enviaEmail !== true && OFFLINE === false) {
            ...
            try {
                $sql = "INSERT INTO usuarios (tmphash, nome, apelido, senha, email, role, acesso, cadastro, validado) VALUES (:tmphash, :nome, :apelido, :senha, :email, :role, :acesso, :cadastro, :validado)";
                $query = $this->db->prepare($sql);
                $parameters = array(':tmphash' => $tmphash, ':nome' => $nome, ':apelido' => $apelido, ':senha' => $hash, ':email' => $email, ':role' => $role, ':acesso' => $acesso, ':cadastro' => $cadastro, ':validado' => $validado);
    ...
    public function login($apelido, $senha) 
    {
        if (!$this->tabelaExiste()) {
            return json_encode(array("tipo"=>"alert-danger","msg"=>"A tabela de usuários não existe."));
        }
    ...
    //         try {
    //             $this->db->exec($sql);
    //             return "Tabela usuarios recriada com sucesso.";
    //         } catch (\PDOException $e) {
    //             return "Erro ao criar tabela: " . $e->getMessage();
    //         }
    //     } else {
    //         return "A tabela usuarios já existe.";
    //     }
    // }
