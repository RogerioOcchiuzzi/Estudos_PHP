...
class BookModel extends AbstractModel {
    ...
    public function __construct(PDO $dbConstruct){
    ...
    public function getByUser(int $userId): array {
        ...
        $query = "SELECT b.* FROM borrowed_books bb LEFT JOIN book b 
            ON bb.book_id = b.id WHERE bb.customer_id = :id";
    ...
    public function returnBook(Book $book, int $userId) {
        ...
        $query = "UPDATE borrowed_books SET end = NOW() WHERE book_id = :book 
            AND customer_id = :user AND end IS NULL";
        $sth = $this->db->prepare($query);
        $sth->bindValue('book', $book->getId());
        $sth->bindValue('user', $userId);
